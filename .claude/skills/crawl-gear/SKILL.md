...
- [ ] `_detailUrl`(상품 개별 상세페이지 URL)이 채워져 있는가? (필수 — 카테고리 리스팅 URL을 넣지 말 것)
- [ ] **색상 옵션이 있는 상품인데 `color`/`colorKorean` 중 하나만 채워지지 않았는가?** 있으면 반드시 둘 다. 사이즈도 동일 — 사이즈 옵션이 있으면 `size`/`sizeKorean` 둘 다 채운다.
- [ ] `imageUrl` HTTPS 절대 URL인가?
- [ ] `weight` 그램 숫자인가? (kg/lb/oz 변환됨). 못 찾았으면 재시도(OCR 등)를 다 해본 뒤에 0으로 뒀는가, 아니면 한 번 시도로 포기했는가?
...
|---|---|---|
| `id` | - | UUID. 문서 ID와 동일 |
| `groupId` | 필수 | 변형 묶음 키 (예: `rab_mythic-ultra-down-sleeping-bag-6c`) |
| `category` | 필수 | 영문 키 33개 중 하나 |
| `company` / `companyKorean` | 필수 | 영문 브랜드 키 / 한글 브랜드명 |
| `nameKorean` | **필수** | 한글 제품명 (사이트에 있는 그대로) |
...
알 수 없으면 빈 문자열 `""` (null 금지). "필수"는 "빈 문자열도 허용 안 됨"을 뜻하고, "선택"은 빈 문자열/0이 정상값일 수 있다는 뜻이다.

### 국내 전용 브랜드 name(영문) 채우기

코베아·제로그램처럼 사이트 자체가 한글만 쓰는 국내 전용 브랜드는 영문 제품명이 존재하지 않는다.
이 경우도 `name`을 빈 문자열로 두지 말고 `nameKorean`을 **로마자 음역**해서 채운다 (예: "라모나 에어썸 UL 400" → "Ramona Airsome UL 400", 고유 모델명은 발음 그대로 로마자 표기, 이미 영문/숫자인 부분(UL, 400 등)은 그대로 둔다). 브랜드명 자체가 이미 로마자(제로그램→zerogram)면 그 표기를 따른다. 기존 "한글화" 절(§ 아래)의 음역 규칙과 방향만 반대일 뿐 같은 방식이다.

### 선택 필드도 여러 번 시도

`weight`처럼 "선택"으로 분류된 필드도 **한 번의 크롤/한 가지 방법으로 못 찾았다고 바로 빈 값으로 확정하지 않는다.** 텍스트에 없으면 이미지 OCR, OCR도 실패하면 다른 상세 이미지(색상별 이미지, 다른 사이즈의 동일 라인업 등)나 사이트의 다른 페이지(모바일 버전, 다른 언어 버전 등)로 재시도한다. 여러 방법을 다 시도하고도 없으면 그때 빈 값/0으로 두고 "데이터 자체가 없음"으로 기록한다(트러블슈팅 절의 "데이터 미존재 구분" 참고) — 재시도 없이 첫 실패로 포기하는 것과는 다르다.
